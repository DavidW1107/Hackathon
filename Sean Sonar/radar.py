...
class Channel:
    ...
    def process(self, X: np.ndarray) -> dict:
        ...
        # adapt background and noise floor only from echo-free bins, so
        # neither absorbs the objects we want to see
        quiet = resid < self.med + 4 * self.sigma
        self.bg[quiet] = (1 - BG_ADAPT) * self.bg[quiet] + BG_ADAPT * seg[quiet]
        if np.count_nonzero(quiet) > 40:
            q = resid[quiet]
            m = np.median(q)
        ...
        # CLEAN: detect the strongest echo, fit its exact complex lobe
        ...
        for _ in range(MAX_OBJ):
            i = int(np.argmax(np.where(killed, 0.0, resid)))
            amp = float(resid[i])
...
class Radar:
    ...
    def _fuse(self, peaks_l: list, peaks_r: list) -> list[dict]:
        ...
        for e_l, snr_l in sorted(peaks_l, key=lambda p: -p[1]):
            best, best_diff = None, 0.30    # max plausible L/R path spread
            ...
            if res < 0.05:                  # ellipses must actually intersect
                used_r.add(best)
