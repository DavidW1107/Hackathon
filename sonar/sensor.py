...
MOTION_THRESH = 0.10          # cross-pulse std (normalised) = movement
...
def process_window(rec2, chirp, flen):
    ...
    clutter = [{"range": round(float(rng[i]), 2), "strength": round(float(mean_n[i]), 2)}
    ...
    for a, b in _clusters((motion_n > MOTION_THRESH) & gate):
        ...
        g = t0 + minlag + peak
        az = float(np.median([_azimuth(rec2[p * flen:(p + 1) * flen, 0][max(g - 200, 0):g + 200],
                                       rec2[p * flen:(p + 1) * flen, 1][max(g - 200, 0):g + 200])
                              for p in range(P)]))
        targets.append({"id": 0, "range": round(float(rng[peak]), 2),
                        "az": round(float(np.clip(az, -FOV, FOV)), 1), "vel": round(speed, 2),
...
def live_loop():
    ...
    emit = np.tile(frame, PULSES_PER_WIN) * 0.8
    t_start = time.monotonic()
    while True:
        rec = sd.playrec(emit, samplerate=FS, channels=2, dtype="float32")
        sd.wait()
        try:
            clutter, targets = process_window(rec, chirp, flen)
        except Exception as e:
            print("process error:", e); continue
        _publish(time.monotonic() - t_start, clutter, targets)
...
def sim_loop():
    ...
    while True:
        ...
        clutter = [{"range": 3.6, "strength": 0.9}, {"range": 1.1, "strength": 0.5},
                   {"range": 4.8, "strength": 0.7}]
